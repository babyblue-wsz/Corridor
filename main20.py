...
if __name__ == '__main__':
    f = open('static/XYZ-20m.txt', 'r', encoding='utf-8')
    ...
    matrix = np.full((715, 1965), 200, dtype=np.int16)

    # todo 读取原xyz文件，获取初始坐标点集合（坐标原点归零处理后的）。原数据 x_min = 489591，y_min = 3490656
    for line in lines:
        temp = line.split(',')

        x = int(float(temp[0]) / 20) - 24510
        y = int(float(temp[1]) / 20) - 174930
    ...
    # 算路径
    total_results = []
    start_position1 = Position(45, 1917)
    target_position1 = Position(177, 1283)
    res1 = SearchPath(slope, start_position1, target_position1)
    if res1:
        for temp in res1:
            total_results.append([(temp[0] + 24510) * 20, (temp[1] + 174930) * 20])
            slope[temp[0], temp[1]] = 15

    start_position2 = Position(172, 1252)
    target_position2 = Position(231, 704)
    ...
    start_position3 = Position(286, 630)
    target_position3 = Position(670, 52)
    ...
    # todo  保存路径点坐标
    note = open('corridor-20m.txt', mode='w', encoding='utf-8')
