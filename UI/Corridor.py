...
from main100 import cal100


class Corridor(QWidget):
    def __init__(self):
    ...
    def init_layers(self):
        ...
        calculation_btn = QPushButton('计算方案')
        top_layout.addWidget(calculation_btn, 0, 2)
        calculation_btn.setFixedWidth(100)
        calculation_btn.clicked.connect(self.calculation_corridor)
    ...
    def open_single_file(self):
        file_path = QFileDialog.getOpenFileName(
            self,  # 父窗口对象
            "选择你要上传的图片",  # 标题
            r"d:\D:\Study\研一下学期\研一下学期项目\安徽项目\2022开始\SlopeCalculate\static",  # 起始目录
            # "图片类型 (*.png *.jpg *.bmp)"  # 选择类型过滤项，过滤内容在括号中
        )
        file_path = file_path[0]
        cal100(file_path)
        res_img_path = r"D:/Study/研一下学期/研一下学期项目/安徽项目/2022开始/SlopeCalculate/Results/res_100.jpg"
        self.input_image.setPixmap(QPixmap(res_img_path))
